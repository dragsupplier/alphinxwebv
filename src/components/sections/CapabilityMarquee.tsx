...
export function CapabilityMarquee() {
  return (
    <section
      id="capabilities"
      className="relative overflow-hidden border-y border-ink-200 bg-white py-14"
    >
      ...
      <div className="mx-auto mb-9 grid max-w-7xl grid-cols-1 items-end gap-5 px-5 md:grid-cols-12 md:px-8">
        <div className="md:col-span-7">
          ...
          <h2 className="mt-4 font-display text-[28px] font-bold leading-[1.05] tracking-[-0.02em] text-ink-950 md:text-[36px]">
            One team. Forty-plus services. Engineered to plug into each other.
          </h2>
        </div>
        <p className="text-[14px] text-ink-500 md:col-span-5 md:text-right md:text-[14.5px]">
...
function Track({
  ...
  return (
    <div className="relative" aria-hidden>
      <span className="absolute left-3 top-1/2 z-20 hidden -translate-y-1/2 rounded-md bg-ink-950 px-2.5 py-1 font-mono text-[10.5px] font-semibold uppercase tracking-[0.18em] text-paper md:left-8 md:inline-block">
        {label}
      </span>
      <div className="overflow-hidden">
        ...
        >
          {repeated.map(({ i: Icon, t }, idx) => (
            <span
              key={`${t}-${idx}`}
              className="group inline-flex shrink-0 items-center gap-2.5 rounded-full bg-paper px-4 py-2.5 text-[14px] font-medium text-ink-800 ring-1 ring-ink-200 transition-colors hover:bg-ink-950 hover:text-paper hover:ring-ink-950"
            >
              <Icon className="h-[16px] w-[16px] text-brand-700 group-hover:text-accent-500" strokeWidth={2} />
              {t}
            </span>
          ))}
        </div>
